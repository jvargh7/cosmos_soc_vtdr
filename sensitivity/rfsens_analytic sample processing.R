sensitivity_analytic <- readRDS(paste0(path_retinopathy_fragmentation_folder,"/working/rfa001_analytic sample.RDS")) %>% 
  ...
  mutate(time_to_death = as.numeric(difftime(DeathDate, diag_date,units = "weeks"))/4) %>% 
  mutate(t_sens = case_when(!is.na(months_to_event) ~ t,
                            !is.na(time_to_death) ~ time_to_death,
                            TRUE ~ t),
         event_type = case_when(!is.na(months_to_event) ~ 1,
                                !is.na(time_to_death) ~ 2,
                                TRUE ~ 0)) %>% 
