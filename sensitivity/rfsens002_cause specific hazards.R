...
mi_dfs <- readRDS(paste0(path_retinopathy_fragmentation_folder,"/working/rfsens001_multiple imputation of sensitivity.RDS"))
...
mice_fit_coxph <- function(mids_dfs, formula = formula,est_contrasts = FALSE
  ...
  fits = mice::as.mira(
    map(complete(mids_dfs,"all"),
        function(df){
          df2 = df %>% 
            left_join(sensitivity_analytic %>% 
            ...
            mutate(outcome_cause_specific = case_when(event_type %in% c(0,2) ~ 0,
                                                      TRUE ~ 1))
...
hba1c_main0 <- mice_fit_coxph(mids_dfs = mi_dfs,formula = paste0(outcome_cause_specific,exposure_main))
hba1c_main1 <- mice_fit_coxph(mids_dfs = mi_dfs,formula = paste0(outcome_cause_specific,exposure_main,covariates))
hba1c_main2 <- mice_fit_coxph(mids_dfs = mi_dfs,formula = paste0(outcome_cause_specific,exposure_interaction,covariates),est_contrasts = TRUE)
