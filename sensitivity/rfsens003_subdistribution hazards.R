...
source("sensitivity/rfsens_analytic sample processing.R")

source("sensitivity/rfsens_competing risk equations.R")
...
# Define a function to fit Competing risk regression and extract robust SE
fit_fgh <- function(data,f) {
  # https://mskcc-epi-bio.github.io/tidycmprsk/
  model <- crr(as.formula(f), data = data)
...
mice_fit_fgh <- function(mids_dfs, formula = formula,est_contrasts = FALSE
){
    ...
    map(complete(mids_dfs,"all"),
        function(df){
          df2 = df %>% 
            ...
            dplyr::filter(!is.na(t_sens),t_sens>=1)
          
          # if(est_emmeans){
          #   fit_coxph_emmeans(df2,f=formula)
          #   
          # }else{
          fit_fgh(df2,f=formula)
  ...
  # https://stackoverflow.com/questions/41794649/can-mice-handle-crr-fine-gray-model
...
hba1c_main0 <- mice_fit_fgh(mids_dfs = mi_dfs,formula = paste0(outcome_subdistribution,exposure_main))
hba1c_main1 <- mice_fit_fgh(mids_dfs = mi_dfs,formula = paste0(outcome_subdistribution,exposure_main,covariates))
hba1c_main2 <- mice_fit_fgh(mids_dfs = mi_dfs,formula = paste0(outcome_subdistribution,exposure_interaction,covariates),est_contrasts = TRUE)
...
(rfa005 <- bind_rows(
  ...
  write_csv(.,"sensitivity/rfsens001_competing risk estimates.csv")
...
contrast_estimates %>% 
  write_csv(.,"sensitivity/rfsens001_contrast of survival estimates.csv")
