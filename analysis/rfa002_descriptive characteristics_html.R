...
analytic %>% 
  mutate(only_medicare = case_when(insurance_medicare == 1 & 
                                     insurance_other == 0 &
                                     insurance_medicaid == 0 ~ 1,
                                   TRUE ~ 0)) %>% 
  group_by(region) %>% 
  summarize(prop = mean(only_medicare))
...
analytic %>% 
  ...
  dplyr::select(c(age,female,raceeth,
                  ...
                  historical_bp_group,historical_ldl_group,
...
(rfa01_descriptives <- analytic %>% 
  ...
  tbl_summary(by=region,
              ...
              type = list(age ~ "continuous",
                          ...
                          SviHouseholdCharacteristicsPctlRankByZip2020_X ~ "continuous2",
                          SviRacialEthnicMinorityStatusPctlRankByZip2020_X ~ "continuous2",
                          ...
                          historical_bp_group  ~ "categorical",historical_ldl_group  ~ "categorical",
